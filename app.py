...
if is_admin and st.session_state.get('modo_gerenciar', False):
    ...
    with tab2:
        ...
        op_j1.metric("Sentenças Procedentes", totais_mes['sentecas_proc'])
        op_j2.metric("Sentenças Improcedentes", totais_mes['sentecas_improc'])
        op_j3.metric("Acordos Homologados", totais_mes['acordos_homologados'])
        op_j4.metric("Recursos Improvidos", totais_mes['judicial_rec_improvidos'])

        op_j5, op_j6, op_j7, op_j8 = st.columns(4, gap="medium")
        op_j5.metric("Extinto sem Resolução", totais_mes['judicial_extinto'])
        op_j6.metric("Perícias Realizadas", totais_mes['judicial_pericia_realizada'])
        op_j7.metric("Prazos no Fatal", totais_mes['prazos_fatal'])
        op_j8.metric("Prazos Perdidos", totais_mes['prazos_perdidos'])

        op_j9, op_j10, _, _ = st.columns(4, gap="medium")
        op_j9.metric("Reprotocolos", totais_mes['judicial_retrabalho'])
        op_j10.metric("Protocolos Iniciais Judiciais", totais_mes['judicial_iniciais'])
